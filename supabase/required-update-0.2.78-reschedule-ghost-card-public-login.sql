...
delete from public.special_card_entries entries
using ghost_card, public.users users
where entries.special_card_id = ghost_card.id
  and entries.user_id = users.id
  and upper(coalesce(users.member_number, '')) not in ('JUJU-000001', 'JUJU-000002', 'JUJU-000005');
